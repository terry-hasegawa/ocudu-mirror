...
public:
  ...
  {
    ocudu_assert(numerology < NOF_NUMEROLOGIES, "Invalid numerology idx={} passed", numerology);
    ocudu_assert(count < nof_slots_per_hyper_system_frame(), "Invalid slot count={} passed", count);
  }
  ...
  {
    ocudu_assert(numerology < NOF_NUMEROLOGIES, "Invalid numerology idx={} passed", numerology);
    ocudu_assert(sfn_val < NOF_SFNS, "Invalid SFN={} provided", sfn_val);
    ocudu_assert(slot_radio_frame_idx_ < nof_slots_per_frame(),
    ...
    count_val(slot_subframe_index_ + subframe_index_ * nof_slots_per_subframe() + sfn_val * nof_slots_per_frame())
  {
    ocudu_assert(numerology < NOF_NUMEROLOGIES, "Invalid numerology idx={} passed", numerology);
    ocudu_assert(sfn_val < NOF_SFNS, "Invalid SFN={} provided", sfn_val);
    ocudu_assert(subframe_index_ < NOF_SUBFRAMES_PER_FRAME, "Invalid Subframe index={} provided", subframe_index_);
    ocudu_assert(slot_subframe_index_ < nof_slots_per_subframe(),
  ...
  /// Number of slots per hyper system frame. Values: (0..1023) * nof_slots_per_frame.
...
template <>
struct formatter<ocudu::slot_point> {
  ...
  auto format(ocudu::slot_point slot, FormatContext& ctx) const
  {
