...
inline unsigned ssb_get_l_first(ssb_pattern_case pattern_case, unsigned ssb_idx)
{
  ...
  // For carrier frequencies smaller than or equal to 3 GHz, n = 0, 1 . For carrier frequencies within FR1 larger than 3
  // GHz, n = 0, 1, 2, 3.
  // - For unpaired spectrum operation
  // For carrier frequencies smaller than 1.88 GHz, n = 0, 1 . For carrier frequencies within FR1 equal to or larger
  // than 1.88 GHz, n = 0, 1, 2, 3.
  ...
  // Impossible!
  ocudu_assert(false, "Invalid SSB pattern case");
  return {};
}
                                ...
                                ssb_subcarrier_offset subcarrier_offset)
{
  ...
  ocudu_assert(is_scs_valid(ssb_scs, fr),
               "Unsupported combination of FR{} and SSB SCS {}kHz.",
               fr == frequency_range::FR1 ? 1 : 2,
               scs_to_khz(ssb_scs));
  ocudu_assert(is_scs_valid(common_scs, fr),
...
/// \return L_max value.
uint8_t ssb_get_L_max(subcarrier_spacing ssb_scs, arfcn_t dl_arfcn, std::optional<nr_band> band = {});
