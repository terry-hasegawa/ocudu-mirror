...
{
  ocudulog::fetch_basic_logger("ASN1").error(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_warning(const char* format, Args&&... args)
{
  ocudulog::fetch_basic_logger("ASN1").warning(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_info(const char* format, Args&&... args)
{
  ocudulog::fetch_basic_logger("ASN1").info(format, std::forward<Args>(args)...);
}

template <typename... Args>
void log_debug(const char* format, Args&&... args)
{
  ocudulog::fetch_basic_logger("ASN1").debug(format, std::forward<Args>(args)...);
}
...
{
  if (OCUDU_UNLIKELY(current_type.value != access_type)) {
...
enum OCUDUASN_CODE {
  OCUDUASN_ERROR             = -3,
  OCUDUASN_ERROR_ENCODE_FAIL = -2,
  OCUDUASN_ERROR_DECODE_FAIL = -1,
  OCUDUASN_SUCCESS           = 0
};
...
#define HANDLE_CODE(ret)                                                                                               \
  do {                                                                                                                 \
    OCUDUASN_CODE macrocode = ((ret));                                                                                 \
    if (macrocode != OCUDUASN_SUCCESS) {                                                                               \
...
struct ValOrError {
  uint32_t      val;
  OCUDUASN_CODE code;
  ValOrError() : val(0), code(OCUDUASN_SUCCESS) {}
  ValOrError(uint32_t val_, OCUDUASN_CODE code_) : val(val_), code(code_) {}
};
...
struct bit_ref {
  bit_ref(ocudu::byte_buffer_writer byte_writer) : writer(byte_writer) {}

  ocudu::byte_buffer_view data() const { return writer.view(); }

  int distance_bytes() const;
  int distance() const;

  OCUDUASN_CODE pack(uint64_t val, uint32_t n_bits);
  OCUDUASN_CODE pack_bytes(ocudu::span<const uint8_t> bytes);
  OCUDUASN_CODE pack_bytes(ocudu::byte_buffer_view bytes);
  OCUDUASN_CODE align_bytes_zero();

private:
  ocudu::byte_buffer_writer writer;
  uint8_t                   offset = 0;
};

// read-only bit_ref
struct cbit_ref {
  cbit_ref(ocudu::byte_buffer_view buffer_) : buffer(buffer_), it(buffer.begin()) {}
  ...
  ocudu::byte_buffer_view data() const { return buffer; }

  int           distance_bytes() const;
  int           distance() const;
  int           distance(const cbit_ref& other) const;
  OCUDUASN_CODE advance_bits(uint32_t n_bits);
  OCUDUASN_CODE advance_bytes(uint32_t bytes);

  template <class T>
  OCUDUASN_CODE unpack(T& val, uint32_t n_bits);
  OCUDUASN_CODE unpack_bytes(ocudu::span<uint8_t> bytes);
  OCUDUASN_CODE align_bytes();

private:
  ocudu::byte_buffer_view            buffer;
  ocudu::byte_buffer::const_iterator it;
  uint8_t                            offset = 0;
};
...
public:
  ...
  {
    ocudu_assert(idx < size(), "out-of-bounds access to dyn_array ({} >= {})", idx, size());
    return data_[idx];
  }
...
struct asn1_null_t {
  OCUDUASN_CODE pack(bit_ref& bref) const { return OCUDUASN_SUCCESS; }
  OCUDUASN_CODE unpack(cbit_ref& bref) const { return OCUDUASN_SUCCESS; }
};
...
OCUDUASN_CODE pack_enum(bit_ref& bref, EnumType e)
{
...
struct EnumPacker {
  ...
  OCUDUASN_CODE pack(bit_ref& bref, EnumType e)
  {
    return pack_enum(bref, e);
  }
  template <class EnumType>
  OCUDUASN_CODE unpack(EnumType& e, cbit_ref& bref)
  {
...
{
  ocudu_assert(e.nof_types == 1, "Can't convert enum with too many values");
...
template <typename IntType>
OCUDUASN_CODE pack_length(bit_ref& bref, IntType n, IntType lb, IntType ub, bool aligned = false);
template <typename IntType>
OCUDUASN_CODE unpack_length(IntType& n, cbit_ref& bref, IntType lb, IntType ub, bool aligned = false);

// Pack as a small non-negative whole number
OCUDUASN_CODE pack_length(bit_ref& ref, uint32_t val, bool aligned = false);
OCUDUASN_CODE unpack_length(uint32_t& val, cbit_ref& ref, bool aligned = false);
...
template <typename IntType>
OCUDUASN_CODE pack_integer(bit_ref& bref,
                           IntType  n,
                           ...
                           bool     aligned = false);
template <typename IntType>
OCUDUASN_CODE unpack_integer(IntType&  n,
                             cbit_ref& bref,
                             IntType   lb      = std::numeric_limits<IntType>::min(),
                             IntType   ub      = std::numeric_limits<IntType>::max(),
                             bool      has_ext = false,
...
// unconstrained case
template <typename IntType>
OCUDUASN_CODE pack_unconstrained_integer(bit_ref& bref, IntType n, bool has_ext = false, bool aligned = false);
template <typename IntType>
OCUDUASN_CODE unpack_unconstrained_integer(IntType& n, cbit_ref& bref, bool has_ext = false, bool aligned = false);

template <class IntType>
struct integer_packer {
  integer_packer(IntType lb_, IntType ub_, bool has_ext_ = false, bool aligned_ = false);
  OCUDUASN_CODE pack(bit_ref& bref, IntType n);
  OCUDUASN_CODE unpack(IntType& n, cbit_ref& bref);
  IntType       lb;
  IntType       ub;
  bool          has_ext;
  bool          aligned;
...
struct BitPacker {
  ...
  template <typename T>
  OCUDUASN_CODE pack(bit_ref& bref, const T& topack)
  {
    bref.pack(topack, nof_bits);
    return OCUDUASN_SUCCESS;
  }
  template <typename T>
  OCUDUASN_CODE unpack(T& tounpack, cbit_ref& bref)
  {
...
uint64_t to_uint(ocudu::span<const uint8_t> buf);
uint64_t to_uint(const ocudu::byte_buffer& buf);

/// Convert unsigned integer to span of octets.
void to_octet_string(ocudu::span<uint8_t> buf, uint64_t number);
void to_octet_string(ocudu::byte_buffer& buf, uint64_t number);

/// Convert octets into hex string.
std::string to_hex_string(ocudu::span<const uint8_t> buf);
std::string to_hex_string(const ocudu::byte_buffer& buf);

/// Convert std::string of hexadecimal numbers to span of bytes.
unsigned hex_string_to_octets(ocudu::span<uint8_t> buf, const std::string& str);

/// Append std::string of hexadecimal numbers to byte_buffer.
void append_hex_string(ocudu::byte_buffer& buf, const std::string& str);
...
public:
  ...
  {
    return octet_string_helper::to_hex_string(ocudu::span<const uint8_t>{data(), size()});
  }
  bounded_octstring<LB, UB, aligned>& from_string(const std::string& hexstr)
  {
    if (hexstr.size() > 2 * UB) {
      ...
      octet_string_helper::hex_string_to_octets(ocudu::span<uint8_t>{octets_.data(), octets_.size()}, hexstr);
    }
    return *this;
  }
  uint64_t to_number() const
  {
    return octet_string_helper::to_uint(ocudu::span<const uint8_t>{octets_.data(), size()});
  }
...
class unbounded_octstring : public ocudu::byte_buffer
{
public:
  ...
  using ocudu::byte_buffer::byte_buffer;
  unbounded_octstring(byte_buffer other) noexcept : ocudu::byte_buffer(std::move(other)) {}
  unbounded_octstring(const unbounded_octstring& other) noexcept;
  unbounded_octstring(unbounded_octstring&& other) noexcept : ocudu::byte_buffer(std::move(other)) {}

  unbounded_octstring& operator=(byte_buffer other) noexcept
  {
    *static_cast<ocudu::byte_buffer*>(this) = std::move(other);
    return *this;
  }
  ...
  unbounded_octstring<Al>& from_bytes(ocudu::span<const uint8_t> bytes);

  uint64_t                 to_number() const;
  unbounded_octstring<Al>& from_number(uint64_t val);

  ocudu::byte_buffer to_byte_buffer() { return std::move(*static_cast<ocudu::byte_buffer*>(this)); }
...
OCUDUASN_CODE pack_fixed_seq_of(bit_ref& bref, const ArrayType& item_array, uint32_t nof_items, ItemPacker packer)
{
...
OCUDUASN_CODE pack_fixed_seq_of(bit_ref& bref, const ArrayType& item_array, uint32_t nof_items)
{
  for (uint32_t i = 0; i < nof_items; ++i) {
...
OCUDUASN_CODE unpack_fixed_seq_of(ArrayType& item_array, cbit_ref& bref, uint32_t nof_items, ItemUnpacker unpacker)
{
  for (uint32_t i = 0; i < nof_items; ++i) {
...
template <class ArrayType>
OCUDUASN_CODE unpack_fixed_seq_of(ArrayType& item_array, cbit_ref& bref, uint32_t nof_items)
{
  for (uint32_t i = 0; i < nof_items; ++i) {
...
template <class ArrayType, class ItemPacker>
OCUDUASN_CODE pack_dyn_seq_of(bit_ref&         bref,
...
template <class ArrayType>
OCUDUASN_CODE pack_dyn_seq_of(bit_ref& bref, const ArrayType& seqof, uint32_t lb, uint32_t ub, bool aligned = false)
{
  HANDLE_CODE(pack_length(bref, seqof.size(), lb, ub, aligned));
  for (uint32_t i = 0; i < seqof.size(); ++i) {
...
template <class ArrayType, class ItemUnpacker>
OCUDUASN_CODE unpack_dyn_seq_of(ArrayType&   seqof,
                                cbit_ref&    bref,
                                uint32_t     lb,
                                uint32_t     ub,
...
template <class ArrayType>
OCUDUASN_CODE unpack_dyn_seq_of(ArrayType& seqof, cbit_ref& bref, uint32_t lb, uint32_t ub, bool aligned = false)
{
  uint32_t nof_items;
  HANDLE_CODE(unpack_length(nof_items, bref, lb, ub, aligned));
  seqof.resize(nof_items);
...
struct SeqOfPacker {
  ...
  template <typename T>
  OCUDUASN_CODE pack(bit_ref& bref, const T& topack) const
  {
...
struct dyn_seq_of : public dyn_array<ItemType> {
  ...
  OCUDUASN_CODE pack(bit_ref& bref) const { return pack_dyn_seq_of(bref, *this, lb, ub, aligned); }
  OCUDUASN_CODE unpack(cbit_ref& bref) { return unpack_dyn_seq_of(*this, bref, lb, ub, aligned); }
};
...
{
public:
  OCUDUASN_CODE pack(bit_ref& bref) const { return asn_string_utils::pack(bref, str, LB, UB, ALB, AUB, ext, aligned); }
  OCUDUASN_CODE unpack(cbit_ref& bref) { return asn_string_utils::unpack(str, bref, LB, UB, ALB, AUB, ext, aligned); }
...
template <class T>
OCUDUASN_CODE unpack_presence_flag(copy_ptr<T>& optional_field, cbit_ref& bref)
{
  bool          present_flag;
  OCUDUASN_CODE ret = bref.unpack(present_flag, 1);
  if (ret != OCUDUASN_SUCCESS) {
    return ret;
  }
...
public:
  ...
  T& get()
  {
    ...
    ocudu_assert(ret != nullptr, "Invalid choice type");
    return *ret;
  }
...
public:
  ...
  [[nodiscard]] OCUDUASN_CODE unpack_next_group();
  ...
  {
    ocudu_sanity_check(next_group_idx > 0, "No group has been unpacked yet");
  ...
  [[nodiscard]] OCUDUASN_CODE consume_remaining_groups(cbit_ref& bref);
...
private:
  ...
  OCUDUASN_CODE unpack_presence_flags();
...
private:
  ...
  ocudu::byte_buffer varlen_buffer;
  bool               align;
};
...
OCUDUASN_CODE pack_unconstrained_real(bit_ref& bref, float n, bool aligned = false);

OCUDUASN_CODE unpack_unconstrained_real(float& n, cbit_ref& bref, bool aligned = false);
...
public:
  float value;
  real_s() = default;
  OCUDUASN_CODE pack(bit_ref& bref) const { return pack_unconstrained_real(bref, value, true); }
  OCUDUASN_CODE unpack(cbit_ref& bref) { return unpack_unconstrained_real(value, bref, true); }
...
{
  ocudu::byte_buffer buf, buf2;
  Msg                msg2;
  asn1::bit_ref      bref(buf), bref3(buf2);

  if (msg.pack(bref) != asn1::OCUDUASN_SUCCESS) {
    log_error_code(OCUDUASN_ERROR_ENCODE_FAIL, __FILE__, __LINE__);
    return -1;
  }

  asn1::cbit_ref bref2(bref);
  if (msg2.unpack(bref2) != asn1::OCUDUASN_SUCCESS) {
    log_error_code(OCUDUASN_ERROR_DECODE_FAIL, __FILE__, __LINE__);
    return -1;
  }
  if (msg2.pack(bref3) != asn1::OCUDUASN_SUCCESS) {
    log_error_code(OCUDUASN_ERROR_ENCODE_FAIL, __FILE__, __LINE__);
    return -1;
  }
