...
class fixed_size_memory_block_pool
{
  ...
  {
    ocudu_assert(nof_blocks > max_local_cache_size(),
                 "The number of segments in the pool must be much larger than the thread cache size ({} <= {})",
                 nof_blocks,
                 max_local_cache_size());
    ocudu_assert(mblock_size > free_memory_block_list::min_memory_block_align(),
...
public:
  ...
  {
    ocudu_assert(sz <= mblock_size, "Allocated node size={} exceeds max object size={}", sz, mblock_size);
  ...
  {
    ocudu_assert(p != nullptr, "Deallocated nodes must have valid address");
