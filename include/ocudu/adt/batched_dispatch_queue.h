...
public:
  ...
  {
    ocudu_sanity_check(not stopped->load(std::memory_order_relaxed), "Detected push after queue has been stopped");
...
private:
  ...
  {
    ocudu_sanity_check(not stopped->load(std::memory_order_relaxed), "Detected concurrent stop and task dispatch");
    value_type val;
