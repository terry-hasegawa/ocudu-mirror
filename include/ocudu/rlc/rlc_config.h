...
template <>
struct formatter<ocudu::rlc_tx_tm_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_tx_tm_config& cfg, FormatContext& ctx) const
  {
...
template <>
struct formatter<ocudu::rlc_rx_tm_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_rx_tm_config& cfg, FormatContext& ctx) const
  {
...
template <>
struct formatter<ocudu::rlc_tm_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_tm_config& cfg, FormatContext& ctx) const
  {
...
template <>
struct formatter<ocudu::rlc_tx_um_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_tx_um_config& cfg, FormatContext& ctx) const
  {
    return format_to(ctx.out(),
...
template <>
struct formatter<ocudu::rlc_rx_um_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_rx_um_config& cfg, FormatContext& ctx) const
  {
...
template <>
struct formatter<ocudu::rlc_um_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_um_config& cfg, FormatContext& ctx) const
  {
    return format_to(ctx.out(), "{} {}", cfg.tx, cfg.rx);
  }
};
...
template <>
struct formatter<ocudu::rlc_tx_am_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_tx_am_config& cfg, FormatContext& ctx) const
  {
    return format_to(ctx.out(),
...
template <>
struct formatter<ocudu::rlc_rx_am_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_rx_am_config& cfg, FormatContext& ctx) const
  {
    return format_to(ctx.out(),
...
template <>
struct formatter<ocudu::rlc_am_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_am_config& cfg, FormatContext& ctx) const
  {
    return format_to(ctx.out(), "{} {}", cfg.tx, cfg.rx);
  }
};
...
template <>
struct formatter<ocudu::rlc_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx)
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const ocudu::rlc_config& cfg, FormatContext& ctx) const
  {
    if (cfg.mode == ocudu::rlc_mode::tm) {
      return format_to(ctx.out(), "{} {}", cfg.mode, cfg.tm);
    }
    if (cfg.mode == ocudu::rlc_mode::um_bidir) {
      return format_to(ctx.out(), "{} {}", cfg.mode, cfg.um);
    }
    if (cfg.mode == ocudu::rlc_mode::am) {
