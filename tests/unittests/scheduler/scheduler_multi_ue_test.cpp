...
public:
  scheduler_multi_ue_test(const multi_ue_test_params& tparams) :
    scheduler_test_simulator(scheduler_test_sim_config{.sched_cfg =
                                                       ...
                                                       .auto_crc = true}),
    test_params(tparams),
    params(cell_config_builder_profiles::create(duplex_mode::TDD, frequency_range::FR1, bs_channel_bandwidth::MHz100))
  {
    ...
    pucch_resource_builder_params pucch_basic_params{.res_set_0_size           = 8,
                                                     .res_set_1_size           = 8,
    ...
    // Fill no traffic UE set.
    {
      ...
      std::shuffle(tmp.begin(), tmp.end(), test_rng::tls_gen());
...
TEST_F(scheduler_buffer_occupancy_test, when_dl_bo_is_set_then_enough_dl_tbs_is_scheduled)
{
  ...
  for (unsigned i = 0; i != test_params.nof_ues; ++i) {
    ue_dl_bos[i] = test_rng::uniform_int<unsigned>(1, 20000);
...
TEST_F(scheduler_buffer_occupancy_test, when_bsr_is_received_then_enough_ul_bytes_are_scheduled)
{
  ...
  for (unsigned i = 0; i != test_params.nof_ues; ++i) {
    ue_bsrs[i] = test_rng::uniform_int<unsigned>(1, 20000);
    ul_bsr_indication_message bsr{to_du_cell_index(0),
                                  to_du_ue_index(i),
                                  to_rnti(0x4601 + i),
                                  bsr_format::LONG_BSR,
