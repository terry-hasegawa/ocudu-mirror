                        ...
                        unsigned                    nof_grid_rb)
{
  ...
  for (unsigned i_symbol = 0, re_offset = 0; i_symbol != MAX_NSYMB_PER_SLOT; ++i_symbol) {
    ...
    bounded_bitset<MAX_NOF_SUBCARRIERS> re_mask(nof_grid_rb * NOF_SUBCARRIERS_PER_RB);
...
protected:
  ...
  {
    interval<unsigned, true> rb_range(1, MAX_NOF_PRBS);
                                            ...
                                            const re_pattern_list&         reserved)
  {
    ...
    unsigned nof_data_re = allocation.get_inclusion_count(0, MAX_NSYMB_PER_SLOT, ~crb_bitmap(MAX_NOF_PRBS));

    // Get the number of RE in the reserved pattern.
    unsigned nof_reserved_re = reserved.get_inclusion_count(0, MAX_NSYMB_PER_SLOT, ~crb_bitmap(MAX_NOF_PRBS));

    // Number of REs in the data pattern which are also reserved.
    unsigned nof_overlapping_re =
        nof_data_re + nof_reserved_re - both.get_inclusion_count(0, MAX_NSYMB_PER_SLOT, ~crb_bitmap(MAX_NOF_PRBS));
    ...
    for (unsigned i_symbol = 0; i_symbol != MAX_NSYMB_PER_SLOT; ++i_symbol) {
      bounded_bitset<MAX_NOF_SUBCARRIERS> re_pattern_symbol(MAX_NOF_SUBCARRIERS);
...
std::shared_ptr<resource_grid_mapper_factory> ResourceGridMapperFixture::mapper_factory   = nullptr;
...
TEST_F(ResourceGridMapperFixture, SinglePort)
{
  ...
  std::unique_ptr<resource_grid> grid = rg_factory->create(1, MAX_NSYMB_PER_SLOT, MAX_NOF_SUBCARRIERS);
  grid->set_all_zero();
  ...
  assert_grid(golden, allocation, grid->get_reader(), MAX_NOF_PRBS);
}
...
TEST_F(ResourceGridMapperFixture, Identity)
{
  for (unsigned nof_streams = 1; nof_streams <= precoding_constants::MAX_NOF_LAYERS; ++nof_streams) {
    // Create resource grid.
    std::unique_ptr<resource_grid> grid = rg_factory->create(nof_streams, MAX_NSYMB_PER_SLOT, MAX_NOF_SUBCARRIERS);
    grid->set_all_zero();
...
TEST_F(ResourceGridMapperFixture, OneLayerTwoPorts)
{
  ...
  // Create resource grid.
  std::unique_ptr<resource_grid> grid = rg_factory->create(2, MAX_NSYMB_PER_SLOT, MAX_NOF_SUBCARRIERS);
...
INSTANTIATE_TEST_SUITE_P(MultiplePrg,
                         ...
                         ::testing::Combine(
                             ...
                             // PRG size in number of RB.
                             ::testing::Values(4, 13, 52, MAX_NOF_PRBS)));
} // namespace
