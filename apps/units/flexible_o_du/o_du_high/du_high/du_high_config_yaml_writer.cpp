...
static void fill_du_high_log_section(YAML::Node node, const du_high_unit_logger_config& config)
{
  ...
  node["ntn_level"]                        = ocudulog::basic_level_to_string(config.ntn_level);
...
static YAML::Node build_cell_entry(const du_high_unit_base_cell_config& config)
{
  ...
  node["pci"]                   = config.pci;
  node["dl_arfcn"]              = config.dl_f_ref_arfcn.value();
  ...
  if (config.ntn_cfg) {
    fill_ntn_config_in_yaml_schema(node, config.ntn_cfg.value());
  }
