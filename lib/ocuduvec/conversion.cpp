...
static void convert_fi_simd(const float* x, int16_t* z, float scale, unsigned len)
{
  ...
  if (SIMD_IS_ALIGNED(x) && SIMD_IS_ALIGNED(z)) {
    ...
    for (; i + OCUDU_SIMD_S_SIZE < len + 1; i += OCUDU_SIMD_S_SIZE) {
      ...
      simd_f_t b = ocudu_simd_f_loadu(x + i + OCUDU_SIMD_F_SIZE);
...
#endif /* OCUDU_SIMD_F_SIZE && OCUDU_SIMD_S_SIZE */
...
static void convert_bf16_to_f_simd(float* out, const bf16_t* in, unsigned len)
{
  ...
  for (unsigned i_end = (len / 4) * 4; i != i_end; i += 4) {
    ...
    // Store the result back to memory.
    ocudu_simd_f_storeu(out + i, output_vec);
  }
#else // __ARM_FEATURE_BF16

#if OCUDU_SIMD_F_SIZE && OCUDU_SIMD_S_SIZE
  for (unsigned end = (len / OCUDU_SIMD_S_SIZE) * OCUDU_SIMD_S_SIZE; i != end; i += OCUDU_SIMD_S_SIZE) {
    simd_f_t even, odd;
    ...
    ocudu_simd_bf16_loadu(even, odd, in + i);
    ocudu_simd_f_storeu_interleaved(out + i, even, odd);
  }
#endif // OCUDU_SIMD_F_SIZE && OCUDU_SIMD_S_SIZE
...
#if OCUDU_SIMD_F_SIZE && OCUDU_SIMD_S_SIZE
  constexpr unsigned FLOATS_PER_ITERATION = 2 * OCUDU_SIMD_F_SIZE;

  for (unsigned end = (len / FLOATS_PER_ITERATION) * FLOATS_PER_ITERATION; i != end; i += FLOATS_PER_ITERATION) {
    simd_f_t float_vec_1 = ocudu_simd_f_loadu(in + i);
    simd_f_t float_vec_2 = ocudu_simd_f_loadu(in + i + OCUDU_SIMD_F_SIZE);

    // Convert float to brain float and store the result back to memory.
    ocudu_simd_bf16_storeu(out + i, float_vec_1, float_vec_2);
  }
#endif // OCUDU_SIMD_F_SIZE && OCUDU_SIMD_S_SIZE
...
static void convert_bf16_to_int16_simd(int16_t* out, const bf16_t* in, float scale, unsigned len)
{
  ...
  for (unsigned end = (len / OCUDU_SIMD_S_SIZE) * OCUDU_SIMD_S_SIZE; i != end; i += OCUDU_SIMD_S_SIZE) {
    ...
    // Load and unpack bf16 values into two vectors of floats: even part of each 32-bit register storing two bf16 values
    // goes into the first simd register, odd part - into the second one.
    ocudu_simd_bf16_loadu(temp_even, temp_odd, in + i);

    // Multiply with the scaling factor.
    simd_f_t s           = ocudu_simd_f_set1(scale);
    simd_f_t scaled_even = ocudu_simd_f_mul(temp_even, s);
...
static void convert_int16_to_bf16_simd(bf16_t* out, const int16_t* in, float scale, unsigned len)
{
  ...
  for (unsigned i_end = (len / 16) * 16; i < i_end; i += 16) {
    ...
    __m512i bf16_vec = ocudu_simd_convert_1f_bf16(float_vec);
...
void ocudu::ocuduvec::convert(span<int16_t> z, span<const cf_t> x, float scale)
{
  ocudu_assert(2 * x.size() == z.size(), "Invalid input or output span sizes");

  convert_fi_simd(reinterpret_cast<const float*>(x.data()), z.data(), scale, z.size());
}

void ocudu::ocuduvec::convert(span<cbf16_t> z, span<const ci16_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");

  convert_int16_to_bf16_simd(
      reinterpret_cast<bf16_t*>(z.data()), reinterpret_cast<const int16_t*>(x.data()), scale, 2 * z.size());
}

void ocudu::ocuduvec::convert(span<ci16_t> z, span<const cbf16_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");

  convert_bf16_to_int16_simd(
      reinterpret_cast<int16_t*>(z.data()), reinterpret_cast<const bf16_t*>(x.data()), scale, 2 * z.size());
}

void ocudu::ocuduvec::convert(span<cf_t> z, span<const int16_t> x, float scale)
{
  ocudu_assert(x.size() == 2 * z.size(), "Invalid input or output span sizes");

  convert_if_simd(reinterpret_cast<float*>(z.data()), x.data(), scale, x.size());
}

void ocudu::ocuduvec::convert(span<float> z, span<const int16_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<int16_t> z, span<const float> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<cf_t> out, span<const cbf16_t> in)
{
  ocudu_assert(in.size() == out.size(), "Invalid input or output span sizes");
  convert_cbf16_to_cf_simd(out.data(), in.data(), in.size());
}

void ocudu::ocuduvec::convert(span<float> out, span<const bf16_t> in)
{
  ocudu_assert(in.size() == out.size(), "Invalid input or output span sizes");
  convert_bf16_to_f_simd(out.data(), in.data(), in.size());
}

void ocudu::ocuduvec::convert(span<cbf16_t> out, span<const cf_t> in)
{
  ocudu_assert(in.size() == out.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<bf16_t> out, span<const float> in)
{
  ocudu_assert(in.size() == out.size(), "Invalid input or output span sizes");
  convert_f_to_bf16_simd(out.data(), in.data(), in.size());
}

void ocudu::ocuduvec::convert(span<int16_t> z, span<const ocudu::cbf16_t> x, float scale)
{
  ocudu_assert(2 * x.size() == z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<cbf16_t> z, span<const int16_t> x, float scale)
{
  ocudu_assert(x.size() == 2 * z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<cbf16_t> z, span<const int16_t> x, span<const float> scale)
{
  ocudu_assert(x.size() == 2 * z.size(), "Invalid input or output span sizes");
  ocudu_assert(x.size() == scale.size(), "Invalid input data or input scaling span sizes");

  convert_scaled_int16_to_bf16_simd(reinterpret_cast<bf16_t*>(z.data()), x.data(), scale.data(), x.size());
}

void ocudu::ocuduvec::convert(span<int16_t> z, span<const bf16_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<bf16_t> z, span<const int16_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<ci16_t> z, span<const cf_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");
...
void ocudu::ocuduvec::convert(span<cf_t> z, span<const ci16_t> x, float scale)
{
  ocudu_assert(x.size() == z.size(), "Invalid input or output span sizes");
