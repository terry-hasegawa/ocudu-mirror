...
bool ue_context_setup_procedure::create_ue_rrc_context(const f1ap_ue_context_setup_response& ue_ctxt_setup_resp)
{
  ...
  if (ue_ctxt_setup_resp.c_rnti.has_value()) {
    ...
    if (ue_ctxt->get_ul_bearer_manager().is_srb_active(srb_id_t::srb0) and
        ue_ctxt->get_ul_bearer_manager().is_srb_active(srb_id_t::srb1) and
        ue_ctxt->get_ul_bearer_manager().is_srb_active(srb_id_t::srb2)) {
                                               ...
                                               const f1ap_ue_ids&                      ue_ids)
{
  ...
  if (request.conditional_inter_du_mobility_info.has_value()) {
    ...
    // [TS 38.473, 9.3.1.x] Target GNB-DU-UE-F1AP-ID is present if CHO-Trigger is cho-replace.
